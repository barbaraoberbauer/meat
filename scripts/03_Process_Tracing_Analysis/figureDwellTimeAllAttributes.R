...
calculateFixPropsDif <- function(df, dataset){
  ...
  fixProps$translation0 <- df$t_consumption_translation0/1000
  fixProps$popularity0 <- df$t_popularity0/1000
  fixProps$consumption1 <- df$t_consumption1/1000
  fixProps$translation1 <- df$t_consumption_translation1/1000
...
# do not plot translations
fixPropsDifOriginal_subset <- fixPropsDifOriginal %>% filter(attribute != "translation0_diff" & attribute != "translation1_diff")

plotOriginal <- plotDwellTimes(fixPropsDifOriginal_subset, "Study 1", labelsOriginal)

# Replication

fixPropsDifReplication_subset <- fixPropsDifReplication %>% filter(attribute != "translation0_diff" & attribute != "translation1_diff")

plotReplication <- plotDwellTimes(fixPropsDifReplication_subset, "Study 2", labelsReplication)


# Plot absolute dwell times ----------

calculateDwellTimeDif <- function(df, dataset){
...
DTDifOriginal <- calculateDwellTimeDif(dfOriginal, "original")
DTDifReplication <- calculateDwellTimeDif(dfReplication, "replication")
...
# do not plot translations
DTDifOriginal_subset <- DTDifOriginal %>% filter(attribute != "translation0_diff" & attribute != "translation1_diff")

plotDTOriginal <- plotDwellTimes(DTDifOriginal_subset, "Study 1", labelsOriginal)

# Replication

DTDifReplication_subset <- DTDifReplication %>% filter(attribute != "translation0_diff" & attribute != "translation1_diff")

plotDTReplication <- plotDwellTimes(DTDifReplication_subset, "Study 2", labelsReplication)
...
combinedDTPlots <-
  original_label + (plotDTOriginal + theme(plot.title = element_blank())) +
  replication_label + (plotDTReplication + theme(plot.title = element_blank())) +
  plot_layout(
    ncol = 2,
    widths = c(0.1, 1),
    guides = "collect",
...
# Save plot
ggsave("figures/figureDTAllAttributes.png", 
       combinedDTPlots, 
       width = 10,
       height = 7,
       units = "in")
...
getDescriptives <- function(df){
  
  descriptives <- df %>%
    group_by(consumption_translation, attribute) %>%
    summarize(meanDT = mean(propDwellTimeDiff, na.rm = TRUE),
              sdDT = sd(propDwellTimeDiff, na.rm = TRUE))
  
  return(descriptives)
  
}

descriptivesOriginal <- getDescriptives(fixPropsDifOriginal)
descriptivesReplication <- getDescriptives(fixPropsDifReplication)

# Calculate t-tests ------

getTTest <- function(df){
  
  results <- df %>%
    group_by(consumption_translation, attribute) %>%
    reframe({
      
      x <- propDwellTimeDiff[!is.na(propDwellTimeDiff)]
...
ttestOriginal <- getTTest(DTDifOriginal_subset)

DTDifReplication_subset2 <- DTDifReplication_subset %>% filter(consumption_translation != "emission_replace" & consumption_translation != "rating_replace")
ttestReplication <- getTTest(DTDifReplication_subset2)
