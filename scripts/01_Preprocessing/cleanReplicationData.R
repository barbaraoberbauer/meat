...
df <- df %>%
  rename(
    priceEco_raw = priceEco,
    priceNonEco_raw = priceNonEco,
    energyEco_raw = energyEco,
    energyNonEco_raw = energyNonEco,
    popularityEco_raw = popularityEco,
    popularityNonEco_raw = popularityNonEco
  )

orig_cols <- c("priceEco_raw",
          "priceNonEco_raw",
          "energyEco_raw",
          "energyNonEco_raw",
          "popularityEco_raw",
          "popularityNonEco_raw")

norm_cols <- sub("_raw$", "", orig_cols)
...
df[norm_cols] <- lapply(df[orig_cols], function(x) normalizeValue(x,
                                                             min(x),
                                                             max(x),
                                                             10,
                                                             1))
